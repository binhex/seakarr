    ...
    pub fn scan_many(roots: &[PathBuf]) -> Self {
        ...
        for root in roots {
            if std::fs::read_dir(root).is_err() {
                crate::warn!("Skipping unreadable shared directory {}", root.display());
                continue;
            }
...
fn scan_root(root: &Path, root_name: &str, files: &mut Vec<SharedFile>, folder_count: &mut usize) {
...
mod tests {
    ...
    fn temp_tree() -> std::path::PathBuf {
        ...
        let root = std::env::temp_dir().join(format!("soulseek-shares-{}-{n}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(root.join("album")).unwrap();
        std::fs::write(root.join("top.mp3"), b"aaaa").unwrap();
        std::fs::write(root.join("album").join("song one.flac"), b"bbbbbb").unwrap();
        std::fs::write(root.join("album").join("song two.flac"), b"cc").unwrap();
        root
    }
    ...
    fn scan_many_skips_unreadable_roots() {
        let root = temp_tree();
        let shares =
            Shares::scan_many(&[std::path::PathBuf::from("/does/not/exist"), root.clone()]);
        assert_eq!(shares.file_count(), 3);
        let _ = std::fs::remove_dir_all(root);
    }
