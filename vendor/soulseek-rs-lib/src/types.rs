...
impl Search {
    ...
    pub fn accept(&mut self, result: SearchResult) {
        ...
        let files: usize = self.results.iter().map(|result| result.files.len()).sum();
...
impl Download {
    ...
    pub const fn is_finished(&self) -> bool {
        matches!(
            self.status,
            DownloadStatus::Completed | DownloadStatus::Failed(_) | DownloadStatus::TimedOut
        )
    }
...
impl std::fmt::Display for SessionLoss {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Disconnected => f.write_str("the connection to the server dropped"),
        }
    }
}
