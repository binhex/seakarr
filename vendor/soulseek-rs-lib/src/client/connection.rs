use super::{
    Arc, Client, ClientContext, ClientOperation, ConnectionType, DownloadPeer, DownloadStatus,
    Listen, Peer, PeerRegistry, Receiver, Result, RwLock, RwLockExt, Sender, ServerActor,
    ServerMessage, Shares, SoulseekRs, TcpStream, error, info, mpsc, thread, trace, warn,
};
...
const LOGIN_RESPONSE_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(45);
        ...
        match peer.connection_type {
            ...
            ConnectionType::F => {
                ...
                let Some(token) = peer.token else {
                    error!("[client] F connection from {} without token", peer.username);
                    return;
                };
                ...
                match download_peer.download_file(client_context.clone(), None, None) {
                    Ok((download, filename)) => {
                        ...
                        match client_context.write_safe() {
                            ...
                            Err(e) => error!("[client] connect_to_peer F write: {}", e),
                        }
                    }
                    Err(e) => {
