use super::{
    Client, PeerMessage, Result, RoomEvent, RoomInfo, RwLockExt, ServerMessage, SharedDirectory,
    SoulseekRs, UserMessage, error,
};
...
impl Client {
    ...
    pub fn send_private_message(&self, username: &str, message: &str) -> Result<()> {
        let handle = self
            .server_handle
            .as_ref()
            .ok_or(SoulseekRs::NotConnected)?;
        let msg = crate::message::server::MessageFactory::build_message_user(username, message);
        handle
    ...
    pub(super) fn send_server_message(&self, message: crate::message::Message) -> Result<()> {
        self.server_handle
            .as_ref()
            .ok_or(SoulseekRs::NotConnected)?
    ...
    pub fn request_room_list(&self) -> Result<()> {
        self.send_server_message(crate::message::server::MessageFactory::build_room_list_request())
    }
    ...
    pub fn join_room(&self, room: &str) -> Result<()> {
        self.send_server_message(crate::message::server::MessageFactory::build_join_room(
            room, false,
        ))
    }
    ...
    pub fn leave_room(&self, room: &str) -> Result<()> {
        self.send_server_message(crate::message::server::MessageFactory::build_leave_room(
            room,
        ))
    }
    ...
    pub fn say_in_room(&self, room: &str, message: &str) -> Result<()> {
        self.send_server_message(crate::message::server::MessageFactory::build_say_chatroom(
            room, message,
        ))
    }
    ...
    pub fn request_user_info(&self, username: &str) -> Result<()> {
        ...
        self.send_server_message(MessageFactory::build_get_user_status(username))?;
    ...
    pub fn browse_user(&self, username: &str) -> Result<()> {
        ...
        if connected {
            if let Some(registry) = registry {
                let _ = registry.send_to_peer(username, PeerMessage::SendMessage(request));
            }
        } else {
            self.context
                .write_safe()?
            ...
            if let Some(handle) = &self.server_handle {
                let _ = handle.send(ServerMessage::GetPeerAddress(username.to_string()));
            }
        }
        Ok(())
    }
    ...
    #[must_use]
    pub fn take_browse_result(&self, username: &str) -> Option<Vec<SharedDirectory>> {
        self.context
            .write_safe()
            .ok()
