use super::{
    Arc, Client, ClientContext, Download, DownloadMetadata, DownloadStatus, Receiver, Result,
    RwLock, RwLockExt, Sender, error, info, mpsc, next_download_token,
};
...
impl Client {
    ...
    pub(crate) fn fail_queued_downloads(
        ...
        for (token, sender) in doomed {
            ...
            context.update_download_with_status(token, DownloadStatus::Failed(reason));
        }
    }
}
...
mod tests {
    ...
    fn download_tokens_are_unique_and_below_the_upload_range() {
        let tokens: Vec<u32> = (0..10_000).map(|_| next_download_token()).collect();
