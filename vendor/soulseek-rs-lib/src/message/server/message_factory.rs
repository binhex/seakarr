...
impl MessageFactory {
    ...
    #[must_use]
    pub fn build_login_message(username: &str, password: &str, version: ClientVersion) -> Message {
    ...
    /// from 1.
    #[must_use]
    pub fn build_place_in_queue_response(filename: &str, place: u32) -> Message {
        Message::new()
            .write_int32(44)
    ...
    #[must_use]
    pub fn build_upload_transfer_request(filename: &str, token: u32, size: u64) -> Message {
        Message::new()
            .write_int32(40)
...
fn test_build_login_message() {
    ...
    let expect: Vec<u8> = [
        ...
        98, 114, 97, 105, 110, 50, 8, 0, 0, 0, 49, 51, 51, 55, 53, 49, 51, 55, 176, 0, 0, 0, 32, 0,
...
fn test_build_upload_transfer_request() {
    ...
    let message = MessageFactory::build_upload_transfer_request("song.mp3", 555, 4096);
...
fn test_build_peer_init_message() {
    ...
    let message = MessageFactory::build_peer_init_message("bob", ConnectionType::P, 7);
...
fn test_build_wishlist_search() {
    ...
    let expect: Vec<u8> = [
        103, 0, 0, 0, 12, 0, 0, 0, 10, 0, 0, 0, 116, 114, 97, 110, 99, 101, 32, 119, 97, 120,
    ]
    .to_vec();
    assert_eq!(expect, message.get_data());
}
