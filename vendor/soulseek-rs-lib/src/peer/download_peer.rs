...
fn resolve_download_path(download: &Download) -> Result<String, DownloadError> {
    ...
    let final_path = expanded_path.join(extract_filename_from_path(&download.filename));
...
impl DownloadPeer {
    ...
    fn apply_transfer_deadlines(stream: &TcpStream) -> Result<(), DownloadError> {
        stream
    ...
    fn begin_pierced_download(
        ...
        let download = download_info.ok_or(DownloadError::TokenNotFound(token_u32))?;
            ...
            match stream.read(&mut read_buffer) {
                ...
                Ok(bytes_read) => {
                    ...
                    if transfer.is_none() && !self.no_pierce {
                        transfer =
                            Some(self.begin_pierced_download(data, stream, client_context)?);
                        continue;
                    }
                    ...
                    if chunk_counter.is_multiple_of(PROGRESS_UPDATE_CHUNKS) {
                        ...
                        Self::report_progress(client_context, dl, part.written, speed);
...
mod tests {
    ...
    fn scratch_dir(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("soulseek-part-{}-{name}", std::process::id()));
