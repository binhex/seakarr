...
use crate::message::server::{GetUserStatsHandler, GetUserStatusHandler, RoomListHandler};
use crate::message::{Handlers, MessageType};
use crate::message::{Message, MessageReader};
use crate::peer::ConnectionType;
use crate::peer::Peer;
use crate::types::{ClientVersion, RoomEvent, RoomInfo, SessionLoss, SessionWatch};
...
fn post_login_messages(
    ...
    let mut messages = vec![
        MessageFactory::build_shared_folders_message(shared_folders, shared_files),
...
impl ServerActor {
    ...
    fn initiate_connection(&mut self) {
        let stream = match TcpStream::connect((self.address.host.as_str(), self.address.port)) {
            Ok(s) => s,
            Err(e) => {
    ...
    fn initialize_dispatcher(&mut self) {
        let (dispatcher_sender, dispatcher_receiver) = std::sync::mpsc::channel::<ServerMessage>();
    ...
    fn handle_message(&mut self, msg: ServerMessage) {
        ...
        match msg {
            ...
            ServerMessage::RoomUserJoined { room, username } => {
                self.forward_room_event(RoomEvent::UserJoined { room, username });
            }
    ...
    fn handle_standing_message(&mut self, message: ServerMessage) {
        match message {
            ServerMessage::WishlistSearch { token, query } => {
                self.queue_message(MessageFactory::build_wishlist_search(token, &query));
            }
            ServerMessage::WishlistInterval(seconds) => {
                self.forward_to_client(ClientOperation::WishlistInterval(seconds));
            }
    ...
    fn handle_connect_to_peer(&self, peer: Peer) {
        if let Some(op) = match peer.connection_type {
            ConnectionType::P | ConnectionType::F => Some(ClientOperation::ConnectToPeer(peer)),
    ...
    fn handle_private_message_received(&self, user_message: UserMessage) {
        ...
        {
            error!("[server] Error forwarding private message to client: {}", e);
        }
    }
    ...
    fn handle_file_search_request(&self, username: String, token: u32, query: String) {
        if let Err(e) = self.client_channel.send(ClientOperation::IncomingSearch {
            username,
            token,
            query,
        }) {
    ...
    fn send_message(&mut self, message: Message) {
        ...
        trace!(
            ...
            message
                .get_message_name(
                    MessageType::Server,
                    u32::from_le_bytes(message.get_slice(0, 4).try_into().unwrap_or_default())
                )
                .map_err(|e| e.to_string())
        );
