...
impl PeerActor {
    ...
    fn handle_message(&mut self, msg: PeerMessage) {
        ...
        match msg {
            ...
            PeerMessage::IncomingPlaceInQueueRequest(filename) => {
                ...
                if let Err(e) = self
                    .client_channel
                    .send(ClientOperation::PlaceInQueueRequested {
                        requester_key,
                        filename,
                    })
                {
                    error!("[peer_actor] forward IncomingPlaceInQueueRequest: {}", e);
                }
            }
    ...
    fn handle_transfer_response(&mut self, token: u32, allowed: bool, reason: Option<String>) {
        let username = self.peer_username();
        debug!(
    ...
    fn handle_serve_upload(&mut self, token: u32, filename: String, size: u64) {
        ...
        let message = MessageFactory::build_upload_transfer_request(&filename, token, size);
        self.send_message(message);
    }
    ...
    fn process_read(&mut self) {
...

        {
            ...
            match self.reader.read_from_socket(stream) {
                ...
                Err(ref e) if e.kind() == io::ErrorKind::UnexpectedEof && self.established => {
                    debug!("[peer:{}] peer closed the connection", self.peer_username());
                    self.disconnect();
                    return;
                }
                Err(e) => {
    ...
    fn on_connection_established(&mut self) {
        let (username, connection_type, token) = match self.peer.read_safe() {
            ...
            Err(e) => {
                error!("[peer_actor] on_connection_established peer lock: {}", e);
                return;
            }
        };
        ...
        if self.outbound {
            let handshake = match token {
                Some(server_token) => MessageFactory::build_pierce_firewall_message(server_token),
                None => {
                    MessageFactory::build_peer_init_message(&self.own_username, connection_type, 0)
                }
            };
